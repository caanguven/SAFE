...
MAX_ANGLE = 360
SAWTOOTH_PERIOD = 2  # Period in seconds
DEAD_ZONE_THRESHOLD = 50  # ADC value threshold for dead zone
...
class PIDController:
    def __init__(self, Kp, Ki, Kd, name='PID', max_rate_of_change=None):
        self.Kp = Kp
        self.Ki = Ki
        self.Kd = Kd
        self.previous_error = 0
        ...
        self.max_rate_of_change = max_rate_of_change  # Max change per second

    def compute(self, error, in_dead_zone=False):
        current_time = time.time()
        delta_time = current_time - self.last_time
        if delta_time <= 0.0:
            delta_time = 0.0001
        ...
        # Rate limiting
        if self.max_rate_of_change is not None:
            max_change = self.max_rate_of_change * delta_time
            delta_output = control_signal - self.previous_output
            if abs(delta_output) > max_change:
                control_signal = self.previous_output + max_change * (1 if delta_output > 0 else -1)
                logging.debug(f"{self.name} PID: Rate limiting applied. New control_signal={control_signal}")

        # Limit output during dead zone
        if in_dead_zone:
            control_signal = max(min(control_signal, 50), -50)  # Limit to ±50%
            logging.debug(f"{self.name} PID: Output clamped to ±50% during dead zone. control_signal={control_signal}")

        self.previous_error = error
        self.previous_output = control_signal
        self.last_time = current_time
...
class MotorController:
    ...
    def read_position(self, mcp):
        ...
        # Dead zone detection
        if raw_value <= DEAD_ZONE_THRESHOLD or raw_value >= (ADC_MAX - DEAD_ZONE_THRESHOLD):
            logging.debug(f"{self.name} is in dead zone with raw_value {raw_value}")
            return None  # Indicate dead zone
        else:
            degrees = (raw_value / ADC_MAX) * 360.0
            self.last_valid_position = degrees
            logging.debug(f"{self.name} Position: {degrees}° (raw: {raw_value})")
            return degrees
    ...
    def move_to_position(self, target, mcp):
        try:
            ...
            if current_position is None:
                # In dead zone, assume position is 0°
                current_position = 0.0
                in_dead_zone = True
                logging.debug(f"{self.name} in dead zone, assuming position {current_position}°")
            else:
                in_dead_zone = False

            error = target - current_position

            # Adjust error for wrapping around
            if error > 180:
                error -= 360
            elif error < -180:
                error += 360

            # Apply slowdown threshold outside dead zone
            if not in_dead_zone and abs(error) <= 20:
                # Reduce motor speed proportionally as it approaches target
                self.pid.max_rate_of_change = 20  # Limit the rate of change
            else:
                self.pid.max_rate_of_change = None  # No rate limiting

            control_signal = self.pid.compute(error, in_dead_zone=in_dead_zone)

            if abs(error) <= 2 and not in_dead_zone:
                self.stop_motor()
                logging.debug(f"{self.name} reached target. Stopping motor.")
                return True
            ...
            speed = min(100, max(15, abs(control_signal)))
            self.pwm.ChangeDutyCycle(speed)
            logging.debug(f"{self.name} Speed: {speed}%")
            return False
        except Exception as e:
...
class MotorControlSystem:
    def __init__(self, mode='normal'):
        ...
        if initial_positions:
            average_initial_position = sum(initial_positions) / len(initial_positions)
        else:
            average_initial_position = 0.0
        self.start_time = time.time()
        self.initial_position_offset = average_initial_position
    ...
    def get_status(self):
        with self.lock:
            ...
            for name, motor in self.motors.items():
                pos = motor.read_position(self.mcp)
                status['motors'][name] = pos if pos is not None else 0.0
    ...
    def generate_sawtooth_position(self):
        elapsed_time = time.time() - self.start_time
        position_in_cycle = (elapsed_time % SAWTOOTH_PERIOD) / SAWTOOTH_PERIOD
        position = (self.initial_position_offset + position_in_cycle * MAX_ANGLE) % MAX_ANGLE
        return position
