...
# CSV Filenames
CSV_WITH_FILTER_MOTOR3 = 'with_spike_filter_motor3.csv'
CSV_WITHOUT_FILTER_MOTOR3 = 'without_spike_filter_motor3.csv'
    ...
    def filter(self, new_value):
        ...
        if self.filter_active:
            ...
            if self.LOWER_VALID_LIMIT <= new_value <= self.UPPER_VALID_LIMIT:
                ...
                # Still in dead zone, ignore the reading
                return None
...
class MotorController:
    def __init__(self, name, in1, in2, pwm, adc_channel, mcp, spike_filter_enabled=True):
        ...
        self.pid = PIDController(Kp=PID_KP, Ki=PID_KI, Kd=PID_KD, name=f'PID-{self.name}')
        self.spike_filter = SpikeFilter(name, enabled=spike_filter_enabled)
        self.previous_control_signal = 0.0  # For rate limiting
        self.encoder_flipped = False  # Adjust based on encoder orientation
    ...
    def move_to_position(self, target):
        try:
            ...
            logging.debug(f"{self.name} Speed: {speed}% (Control Signal: {control_signal:.2f}%)")
...
def main(spike_filter_enabled, csv_filename):
    ...
    motor3_pwm = GPIO.PWM(MOTOR3_SPD, 1000)  # 1 kHz frequency
    motor3_pwm.start(0)  # Start PWM with 0% duty cycle
    ...
    motor3 = MotorController("M3", MOTOR3_IN1, MOTOR3_IN2, motor3_pwm,
                             MOTOR3_ADC_CHANNEL, mcp, spike_filter_enabled=spike_filter_enabled)
    ...
    try:
        while (time.time() - start_time) < TEST_DURATION:
            # Generate target position (increasing sawtooth wave for opposite direction)
            ...
            # Move motor to target position in positive direction (forward)
            finished, error, control_signal, actual_position = motor3.move_to_position(target_position)

            # Collect data
            data_records.append({
                'Time (s)': round(elapsed_time, 2),
                'Error (degrees)': round(error, 2),
                'Control Signal (%)': round(control_signal, 2),
                'Target Position (degrees)': round(target_position, 2),
                'Actual Position (degrees)': round(actual_position, 2)
            })
        ...
        with open(csv_filename, 'w', newline='') as csvfile:
            fieldnames = ['Time (s)', 'Error (degrees)', 'Control Signal (%)',
                          'Target Position (degrees)', 'Actual Position (degrees)']
...
if __name__ == "__main__":
    ...
    logging.info("Starting test with spike filter enabled for Motor3.")
    main(spike_filter_enabled=True, csv_filename=CSV_WITH_FILTER_MOTOR3)

    # Run with spike filter disabled
    logging.info("Starting test with spike filter disabled for Motor3.")
    main(spike_filter_enabled=False, csv_filename=CSV_WITHOUT_FILTER_MOTOR3)

    logging.info("All tests completed for Motor3.")
