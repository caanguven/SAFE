...
class MotorController:
    ...
    def set_direction(self, direction):
        """Set the motor's movement direction."""
        if direction == 'forward':
            ...
            self.direction = 'stop'
        print(f"{self.name}: Direction set to {self.direction}")
    ...
    def pid_control_motor(self, degrees_value, set_position):
        ...
        if degrees_value is None:
            if not self.in_dead_zone:
                ...
                if self.last_degrees_value is not None:
                    ...
                    self.estimated_position = 0.0  # Default value
                ...
                if speed is None or speed == 0:
                    speed = self.config.get('default_speed', 10)  # Default to 10% speed
                    print(f"{self.name}: speed is None or zero, using default speed {speed}%")
        ...
        # Ensure degrees_value is not None
        if degrees_value is None:
            degrees_value = self.last_degrees_value if self.last_degrees_value is not None else 0.0
        ...
        # Apply control signal based on error and direction
        if error == 0:
            ...
            # Move motor based on the current direction
            ...
            if self.direction == 'forward':
                ...
                self.motor.backward()
                print(f"{self.name}: Moving backward: Potentiometer Value: {degrees_value:.2f}°, Error: {error:.2f}°, Control Signal: {control_signal:.2f}%, Set Position: {set_position:.2f}°")
            else:
                # Default to forward if direction is not set
                self.motor.forward()
                print(f"{self.name}: Moving forward (default): Potentiometer Value: {degrees_value:.2f}°, Error: {error:.2f}°, Control Signal: {control_signal:.2f}%, Set Position: {set_position:.2f}°")
