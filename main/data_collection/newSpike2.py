...
class MotorControlSystem:
    def __init__(self, mode='normal', run_duration=35, spike_filter_duration=15, catch_up_duration=5):
        ...
        self.csv_writer.writerow(['Time (s)', 'Error (degrees)', 'Control Signal (%)', 'Target Position (degrees)', 'Actual Position (degrees)'])
    ...
    def set_direction(self, direction):
        with self.lock:
            ...
            if direction == 'stable':
                logging.info("Resetting Motor 3 to 'stable' state.")
        ...
        while self.running:
            ...
            if not self.initialized:
                with self.lock:
                    ...
                    logging.info(f"Synchronization complete. Start time adjusted with t0={t0:.2f}s to align sawtooth wave with current position={current_position:.2f}°.")

            # **Catch-Up Phase: First 5 seconds**
            if not self.logging_enabled and elapsed_time >= self.catch_up_duration:
                self.logging_enabled = True
                logging.info(f"Catch-up phase complete. Logging and recording data now enabled.")
            ...
            if direction != 'stable':
                ...
                if log and result[1] is not None:
                    ...
                    self.csv_writer.writerow([f"{elapsed_time:.2f}", f"{error:.2f}", f"{speed:.2f}", f"{target_pos:.2f}", f"{actual_pos:.2f}"])

            else:
                # Stop Motor 3
                self.motors['M3'].stop_motor()
                logging.debug("Motor 3 stopped (direction: stable).")
    ...
    def stop(self):
        if self.running:
            ...
            GPIO.cleanup()
            self.csv_file.close()
            logging.info("MotorControlSystem stopped, GPIO cleaned up, and CSV file closed.")
