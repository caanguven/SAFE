...
def main():
    ...
    try:
        ...
        motor2 = MotorController("Motor 2", MOTOR2_IN1, MOTOR2_IN2, motor2_pwm,
                                 MOTOR2_ADC_CHANNEL, args.motor3_target, encoder_flipped=True)
        motor3 = MotorController("Motor 3", MOTOR3_IN1, MOTOR3_IN2, motor3_pwm,
                                 MOTOR3_ADC_CHANNEL, args.motor3_target, encoder_flipped=False)
        motor4 = MotorController("Motor 4", MOTOR4_IN1, MOTOR4_IN2, motor4_pwm,
                                 MOTOR4_ADC_CHANNEL, args.motor1_target, encoder_flipped=True)
        ...
        while True:
            ...
            print(f"Motor 1 - Target: {group1_targets[0]:.1f}°, Current: {m1_pos:.1f}°")
            print(f"Motor 3 - Target: {group1_targets[1]:.1f}°, Current: {m3_pos:.1f}°")
            print(f"Motor 2 - Target: {group2_targets[0]:.1f}°, Current: {m2_pos:.1f}°")
            print(f"Motor 4 - Target: {group2_targets[1]:.1f}°, Current: {m4_pos:.1f}°")
            print(f"Phase Difference M1-M2: {abs(m1_pos - m2_pos):.1f}°, M3-M4: {abs(m3_pos - m4_pos):.1f}°")
